...
@WebServlet(urlPatterns = "/FoodFindServlet")
public class FoodFindServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String sqlCount="select count(*) from food where FOOD_NAME=? AND USER_NAME=?";
        User user = (User) request.getSession().getAttribute("SESSION_LOGIN");
        totalRows=foodDAO.getFoodCount(sqlCount,foodName,user.getUserName());
        ...
        StringBuffer sqlRow=new StringBuffer("SELECT \n" +
                ...
                "USER_NAME userName\n" +
                "FROM \n" +
                "food where FOOD_NAME=? AND USER_NAME=?\n");
        sqlRow.append(" limit ").append(startRow).append(",").append(pageSize);
        System.out.println(sqlRow.toString());

        List<Food> foodList= foodDAO.getFoodListByPage(sqlRow.toString(),foodName,user.getUserName());
